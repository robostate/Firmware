...
void CameraInterfacePWM::setup()
{
	...
	// Get parameters
	param_get(param_find("TRIG_PWM_SHOOT"), &pwm_camera_shoot_);
	param_get(param_find("TRIG_PWM_NEUTRAL"), &pwm_camera_neutral_);

	// Set neutral pulsewidths
	for (unsigned i = 0; i < arraySize(_pins); i++) {
		if (_pins[i] >= 0) {
			up_pwm_trigger_set(_pins[i], math::constrain(pwm_camera_neutral_, 0, 2000));
		}
	}
...
void CameraInterfacePWM::trigger(bool trigger_on_true)
{
	for (unsigned i = 0; i < arraySize(_pins); i++) {
		if (_pins[i] >= 0) {
			...
			up_pwm_trigger_set(_pins[i], math::constrain(trigger_on_true ? pwm_camera_shoot_ : pwm_camera_neutral_, 0, 2000));
		}
	}
}
