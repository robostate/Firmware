    ...
    stage('Test') {
      parallel {
        ...
        stage('ROS vtol standard mission') {
          ...
          post {
            always {
              sh 'px4-posix_sitl_default*/px4/Tools/upload_log.py -q --description "${JOB_NAME}: ${STAGE_NAME}" --feedback "${JOB_NAME} ${CHANGE_TITLE} ${CHANGE_URL}" --source CI .ros/rootfs/log/*/*.ulg'
